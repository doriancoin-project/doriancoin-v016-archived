...
uint256 hashGenesisBlock("0xd21da25e277bd20b7456087d69c5fee2ebc6091b410271b5cb0623c7d1e7d1b9");
...
{
    // Doriancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 525949 blocks, which will occur approximately every 1 year
    nSubsidy >>= (nHeight / 840000); // Doriancoin: 525949 blocks in ~1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Doriancoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Doriancoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Doriancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Doriancoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x707769464eb59fdd7b75cdbc5f0e72226345281852325c965b8ee1fd592fbf51");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1394325760;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 385834689;

        if (fTestNet)
        {
            block.nTime    = 1394325759;
            block.nNonce   = 149343;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa27b7d0a286e46fae3cb7e5b1eae6001fc1b15afee2f6a147291e7eb19746d5d"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DoriancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DoriancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DoriancoinMiner(CWallet *pwallet)
{
    printf("DoriancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("doriancoin-miner");
    ...
    try { loop {
        ...
        printf("Running DoriancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DoriancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DoriancoinMiner, pwallet));
}
