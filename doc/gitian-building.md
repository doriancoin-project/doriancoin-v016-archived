...
*Setup instructions for a Gitian build of Doriancoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Doriancoin
...
- [Building Doriancoin Core](#building-doriancoin-core)
...
Clone the git repositories for doriancoin and Gitian.
...
in the doriancoin repository under 'Fetch and create inputs' to install sources which require
...
To build Doriancoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the doriancoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/doriancoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/doriancoin-project/doriancoin
...
For example:
```bash
URL=https://github.com/thrasher-/doriancoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit doriancoin=${COMMIT} --url doriancoin=${URL} ../doriancoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit doriancoin=${COMMIT} --url doriancoin=${URL} ../doriancoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit doriancoin=${COMMIT} --url doriancoin=${URL} ../doriancoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the doriancoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../doriancoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/doriancoin
SIGPATH=/some/root/path/doriancoin-detached-sigs

./bin/gbuild --url doriancoin=${BTCPATH},signature=${SIGPATH} ../doriancoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/doriancoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/doriancoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/doriancoin-osx-build.assert
```
...
[doriancoin-project/gitian.sigs.DRC](https://github.com/doriancoin-project/gitian.sigs.DRC/) repository, or if that's not possible create a pull
