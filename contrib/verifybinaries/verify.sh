...
###   doriancoincore.org and doriancoin.org and compares them.
...
WORKINGDIR="/tmp/doriancoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://doriancoincore.org"
HOST2="https://doriancoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="doriancoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'doriancoin-',
...
if [ $? -ne 0 ]; then
   echo "doriancoin.org failed to provide signature file, but doriancoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "doriancoin.org and doriancoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Doriancoin Core binary release signing key installed?"
   fi
