...
The RPM spec file provided here is for Doriancoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://doriancoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Doriancoin git repository](https://github.com/doriancoin/doriancoin) but are not part of
...
Currently only `Source10` is used, for the example `doriancoin.conf` file.
...
in the doriancoin git repository. At present only an SVG version of the Doriancoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the doriancoin
...
needed when 0.12.1 is released, a proper fix is already in the Doriancoin git
master branch.
...
idea to build Doriancoin Core against a static version of that library compiled
...
The `doriancoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `doriancoin-server` package. At this point, most RPM based
...
The `doriancoin-server` package should have SELinux support. How to properly do
...
Doriancoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/doriancoin/). He has been
packaging Doriancoin for Fedora at least since 2012.
...
4. I renamed the `doriancoin` package that contains the Qt GUI to `doriancoin-core` as
that appears to be how the general population refers to it, in contrast to
`doriancoin-xt` or `doriancoin-classic`. I wanted to make sure the general population
