#!/bin/bash
# Copyright (c) 2013 The Doriancoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Doriancoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Doriancoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Doriancoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Doriancoin 0.7 datadir."
    ;;
  3)
    echo "Detected Doriancoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Doriancoin 0.8 datadir."
    ;;
esac
